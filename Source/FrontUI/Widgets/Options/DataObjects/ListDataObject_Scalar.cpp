...
void UListDataObject_Scalar::SetCurrentValueFromSlider(float InValue)
{
	...
	{
		const float ClampedValue = FMath::GetMappedRangeValueClamped(DisplayValueRange, OutputValueRange, InValue);
...
bool UListDataObject_Scalar::TryResetBackToDefaultValue()
{
	if (CanResetBackToDefaultValue())
	{
		if (DataDynamicSetter)
		{
			DataDynamicGetter->SetValueFromString(GetDefaultValueAsString());
